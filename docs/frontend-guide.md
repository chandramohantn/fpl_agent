...
- macOS note: `libomp` installed via Homebrew (`brew install libomp`) for LightGBM support
...
| Step | Command | Verify |
|------|---------|--------|
| Create environment | `uv venv --python 3.12 .venv` | `.venv` created with Python 3.12 |
| Install project | `uv sync` (or `pip install -e ".[dev]"`) | `python -c "import pandas, polars, pyarrow, lightgbm"` works |
